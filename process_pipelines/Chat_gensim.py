...
def line2wordvec(list_line, model, sentend):
    """
    处理传入的切割好的一行的list
    ...
    for i in range(len_list_line if len_list_line <= len_limit else len_limit):
        list_line[i] = model[list_line[i]] if list_line[i] in model.index_to_key else sentend # 将前14个word其转换为vec
    return list_line
...
def get_answer(input_str, model, sentend, chat_model):
    qusetion_list = input2list(input_str)
    question_vec = np.array([line2wordvec(qusetion_list, model, sentend)], dtype=np.float32)
    predictions = chat_model.predict(question_vec)
    return predictions
    # answer_list = 


def chat_gensim(source_name):
    ...
    while(1):
        input_str = input("请输入：")
        answer = get_answer(input_str, model, sentend, chat_model)
        print(answer.shape)
...
#%%
# import gensim.models
# model = gensim.models.KeyedVectors.load_word2vec_format('../tmp/tieba_word2vec_gensim.bin', binary=True)
