...
# %%
def get_dict(source_name, embedding_size):
...
def line2wordvec(list_line, word_dim, sentend, unk, word_vector_dict, len_limit):
    ...
    return list_line
# %%
def qaword2vec(source_name, line_limit, len_limit, embedding_size):
    """
    读取问题和答案的文件
    转换为向量形式进行保存
    返回转换后的list
    """
    word_vector_dict = get_dict(source_name, embedding_size) # 导入字典
...
def seq2seq(source_name, X_vector, Y_vector, epoch_time, embedding_size):
    ...
    timesteps = X_train.shape[1] # 每句话词的长度
    word_dim = X_train.shape[2] # 词向量长度
    print(X_train.shape) # 问题个数, 单词个数, 词向量长度
    print(timesteps)
    print(word_dim)
    ...
    model.save(Config.get_path(source_name, Config.File_Kind.Model, embedding_size, "gensim", epoch_time))
...
# %%
def train_model_gensim(source_name, line_limit, len_limit, epoch_time, embedding_size):
    qlist, alist = qaword2vec(source_name, line_limit, len_limit, embedding_size)
    # print(len(qlist),'\n\n\n', qlist[0],'\n\n\n', qlist[-1])
    print("问答list成功向量化")
