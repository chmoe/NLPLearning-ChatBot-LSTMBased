...
def seq2seq(source_name, X_vector, Y_vector, epoch_time):
    ...
    # model.add(LSTM(units=word_dim, input_shape=X_train.shape[1:], return_sequences=False))
    model.add(LSTM(units=word_dim, input_shape=X_train.shape[1:],return_sequences=True))
    model.add(LSTM(units=word_dim, input_shape=X_train.shape[1:], dropout=0.2,recurrent_dropout=0.2, activation='relu', return_sequences=True))
    model.add(Dense(units=word_dim, activation='softmax'))
    ...
    if os.path.exists(Config.path_model_tmp_finder + Config.get_path_with_kind(source_name) + '/checkpoint'):
        ...
        model.fit(X_train, Y_train, batch_size=32, epochs=epoch_time, validation_split=0.1, callbacks=[cp_callback, reduce_lr])
