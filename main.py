...
if __name__ == '__main__':
    ...
    epoch_time = 10000 # 学习次数

    if source_name == Config.Source_Name.Mem: remove_file(use_gensim=use_gensim, source_name=source_name, qasplit=False, word2vec=True, train_tmp=True, train_model=True)
    if source_name == Config.Source_Name.Douban_multi: remove_file(use_gensim=use_gensim, source_name=source_name, qasplit=False, word2vec=False, train_tmp=True, train_model=True)
    if source_name == Config.Source_Name.Xiaohuangji: remove_file(use_gensim=use_gensim, source_name=source_name, qasplit=False, word2vec=True, train_tmp=True, train_model=True)
    ...
    if use_gensim:
        ...
        chat_gensim(source_name, len_limit) # 使用gensim词向量的模型 回答
